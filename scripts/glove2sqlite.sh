...
awk 'BEGIN { print("BEGIN TRANSACTION;") } { gsub(/^<|>$/, "", $1); sub(/ /, "\t", $0); match($0, /\t(.*)/, arr); gsub(/'"'"'/, "'"'"''"'"'", $1); print("INSERT INTO data VALUES ('"'"'" $1 "'"'"', '"'"'" arr[1] "'"'"');") } END { print("COMMIT TRANSACTION;") }' <"${target_file}" >"${temp_dir}/data.tsv" | sqlite3 "${db_file}" ".mode tabs
.import '|cat -' data"
